...
Looking at the grade_data broadly, do you think that the differences between the expected and observed frequencies are meaningful? Why or why not?
...
```{r}
chisq_result <- chisq.test(REPLACE_ME, p = REPLACE_ME)
chisq_result
```
...
```{r}
alpha <- 0.05
df <- length(REPLACE_ME) - REPLACE_ME
...
If the chi-square statistic is greater than the critical value, we reject the null hypothesis at the 0.05 alpha level. Do we?
...
Describe the differences in the expected and observed frequencies for each grade category. Are there any notable discrepancies? What questions would you ask either the instructor or college about these discrepancies?
